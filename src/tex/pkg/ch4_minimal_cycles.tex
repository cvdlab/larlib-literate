...
\section{Angles functions}
\label{sec:angles_fn}

\subsection{$d=2$}

When in $d=2$, we have a vertex as ($d-2$)-cell and an edge as ($d-1$)-cell.
This function uses the Julia's \texttt{atan2} built-in function to calculate
the angle of the edge from the vertex point of view.
